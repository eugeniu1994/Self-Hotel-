...
namespace AegisImplicitMail
{
    ...
    public class MimeMailer : SmtpSocketClient, IMailer 
    {
        ...
        //public MimeMailer(string host, int port = 465, string userName = null, string passWord ="", SslMode sslType = SslMode.None, AuthenticationType authenticationType = AuthenticationType.PlainText)
        public MimeMailer(
            string host, 
            int port, 
            string userName, 
            ...
            : base(host, port, null, null, AuthenticationType.Base64, true, null, null, SslMode.None)
        {
            Host = host;
            Port = port;
            User = userName;
        ...
        public MimeMailer(string host)
            : base(host, 465, null, null, AuthenticationType.Base64, true, null, null, SslMode.None)
        {
            
        }

        public MimeMailer(string host, int port)
            : base(host, port, null, null, AuthenticationType.Base64, true, null, null, SslMode.None)
        {
        }

        public MimeMailer() 
            : base(null, SslMode.None)
        {
        }
        ...
        public AbstractMailMessage GenerateMail(
            IMailAddress sender, 
            List<IMailAddress> toAddresses, 
            List<IMailAddress> ccAddresses, 
            List<IMailAddress> bccAddresses, 
        ...
        public void Send(AbstractMailMessage message, SendCompletedEventHandler onSendCallBack)
        {
                ...
                using (var client = new SmtpSocketClient(null, SslMode.None))
                {
                    Host = Host;
                    Port = Port;
